...
## [2026-01-19] 预览移除关闭按钮并恢复圆点居中
- 背景/需求：关闭按钮影响美观且导致圆点不居中
- 修改类型：fix
- 影响范围：前端图片预览组件
- 变更摘要：
  1) 移除预览底部关闭按钮
...
- 检索与复用策略：
  - 检索关键词：preview-toolbar / preview-dot / PostImagesGrid
  - 找到的旧实现：预览底部圆点居中布局
  - 最终选择：回退到居中布局，保持其他交互不变
- 风险点：
  - 仅依赖点击遮罩/ESC 退出预览
- 验证方式：
  - 预览底部圆点居中显示，关闭按钮不再出现

## [2026-01-19] 修复预览键盘切换与滚轮缩放卡住
- 背景/需求：PC 端滚轮缩放后无法继续缩放/无法退出，方向键仍滚动列表
- 修改类型：fix
- 影响范围：前端图片预览组件
- 变更摘要：
  1) 键盘事件监听改为 document 捕获模式，避免被默认滚动吞掉
  2) 滚轮缩放事件绑定到 movable-view，保证缩放后仍能继续触发
  3) 额外支持 Esc 关闭预览
- 涉及文件：
  - `sanguidaily-front/src/components/PostImagesGrid.vue`
- 检索与复用策略：
  - 检索关键词：preview / keydown / wheel / movable-view
  - 找到的旧实现：PostImagesGrid 预览缩放/偏移与滑动逻辑
  - 最终选择：复用现有缩放/偏移状态，补充事件入口
- 风险点：
  - 非 H5 平台不一定支持键盘/滚轮事件
- 验证方式：
  - 电脑端预览：方向键切图，滚轮持续缩放，ESC 可退出

## [2026-01-19] 预览支持键盘切换与滚轮缩放（PC 端）
- 背景/需求：电脑端浏览多图动态时需要键盘切换与滚轮缩放
- 修改类型：feat
- 影响范围：前端图片预览组件
- 变更摘要：
  1) 预览打开后监听键盘方向键，支持上一张/下一张/首张/末张
  2) 预览支持滚轮缩放并以鼠标指向点为中心调整偏移
  3) 关闭预览时解除键盘监听，避免影响其他页面
- 涉及文件：
  - `sanguidaily-front/src/components/PostImagesGrid.vue`
- 检索与复用策略：
  - 检索关键词：preview / PostImagesGrid / swiper / movable-view
  - 找到的旧实现：PostImagesGrid 自定义预览浮层与缩放逻辑、PostCard 预览开关事件
  - 最终选择：复用现有 preview 组件与缩放偏移算法，仅补充键盘/滚轮入口
- 风险点：
  - 键盘/滚轮事件仅在 H5/PC 环境生效，非 H5 平台不会触发
- 验证方式：
  - 电脑端打开预览：方向键切换；滚轮缩放围绕鼠标指向

## [2026-01-19] 资源清理列表展示缩略图
- 背景/需求：资源清理页仅显示路径，难以识别具体图片
- 修改类型：feat
- 影响范围：前端资源清理页面
- 变更摘要：
  1) 列表增加缩略图列，图片直接用 path 渲染
  2) 视频类资源显示占位标识，避免空白
- 涉及文件：
  - `sanguidaily-front/src/pages/uploads-cleanup/index.vue`
- 检索与复用策略：
  - 检索关键词：uploads-cleanup / image / resolveAssetUrl / preview
  - 找到的旧实现：composer 图片预览、feed/pinned 图片渲染、contact 头像/二维码渲染
  - 最终选择：复用 resolveAssetUrl 与现有 image 渲染方式，仅补充缩略图布局
- 风险点：
  - 图片路径若无效将显示空白缩略图
- 验证方式：
  - 作者入口 -> 资源清理：列表展示缩略图与视频占位
...
- 变更摘要：
  1) 补充项目简介、技术栈与目录结构说明
  2) 增加前后端启动、关键配置与部署注意事项
  3) 标注运维脚本与安全配置提示
- 涉及文件：
  - `README.md`
- 检索与复用策略：
  - 检索关键词：README / 部署 / 启动 / 配置 / nginx
  - 找到的旧实现：根 README 简述、前端 README、scripts README、后端配置文件、Nginx 示例配置
  - 最终选择：复用现有文档与配置，补齐根 README
- 风险点：
  - 文档可能与后续配置变更产生偏差
- 验证方式：
  - 目视检查文档结构与路径引用（未执行）
...
- 检索与复用策略：
  - 检索关键词：composer / status / is_pinned
  - 找到的旧实现：编辑页始终显示可见性
  - 最终选择：复用编辑页逻辑，增加置顶态控制
- 风险点：
  - 无
- 验证方式：
  - 编辑置顶动态时看不到可见性选项

## [2026-01-13] 置顶动态禁止私密与左滑按钮按状态显示
- 背景/需求：置顶动态不能设为私密，左滑按钮需按置顶状态显示
- 修改类型：fix
- 影响范围：前端首页左滑操作
- 变更摘要：
  1) 置顶动态左滑隐藏“私密”按钮
...
- 涉及文件：
  - `sanguidaily-front/src/pages/feed/index.vue`
- 检索与复用策略：
  - 检索关键词：PostSwipeItem / toggle-visibility / is_pinned
  - 找到的旧实现：左滑按钮固定显示且未拦截置顶私密
  - 最终选择：在首页左滑逻辑内按状态控制
- 风险点：
  - 若后端仍允许置顶私密，需接口层进一步校验
- 验证方式：
  - 置顶动态左滑不显示私密按钮
  - 尝试私密置顶动态提示并不生效

## [2026-01-13] 左滑取消置顶文案调整
- 背景/需求：取消置顶按钮文案改为“取置”
- 修改类型：fix
- 影响范围：前端左滑按钮
- 变更摘要：
...
- 影响范围：前端首页 / 置顶页
- 变更摘要：
  1) 首页置顶操作成功后提示“已置顶/已取消置顶”
  2) 置顶页置顶操作成功后提示同样文案
- 涉及文件：
  - `sanguidaily-front/src/pages/feed/index.vue`
  - `sanguidaily-front/src/pages/pinned/index.vue`
- 检索与复用策略：
  - 检索关键词：togglePin / toast / pinned
  - 找到的旧实现：仅失败提示，无成功提示
  - 最终选择：复用现有 toast 方式追加成功提示
- 风险点：
  - 无
- 验证方式：
  - 置顶/取消置顶后弹出提示
...
- 影响范围：前端置顶页
- 变更摘要：
  1) 年份标识改为左对齐黑色粗体，无背景装饰
  2) 首条置顶动态前强制插入最新年份标识
  3) 月/日改为“DD + MM月”两行样式，移除红色装饰
- 涉及文件：
  - `sanguidaily-front/src/pages/pinned/index.vue`
- 检索与复用策略：
  - 检索关键词：pinned / 时间轴 / year card / time column
  - 找到的旧实现：置顶页时间轴与年份卡片样式
  - 最终选择：在原页面结构内调整样式与时间渲染
- 风险点：
  - 小屏幕左侧时间列宽度需关注
- 验证方式：
  - 置顶页首条显示年份卡片
...
## [2026-01-13] 首页时间序排序与置顶页年份分隔卡片
- 背景/需求：首页列表按时间顺序展示且不标识置顶；置顶页时间轴改为月日展示并插入年份卡片
- 修改类型：feat
- 影响范围：前端首页 / 置顶页 / 动态卡片
- 变更摘要：
  1) 首页列表按创建时间排序，不再因置顶置前
  2) 首页列表隐藏置顶标记
  3) 置顶页时间显示为“MM月DD日”，跨年份插入年份卡片
  4) 置顶页时间轴样式重做，点击条目进入详情页
- 涉及文件：
  - `sanguidaily-front/src/stores/postStore.js`
  - `sanguidaily-front/src/components/PostCard.vue`
  - `sanguidaily-front/src/pages/feed/index.vue`
  - `sanguidaily-front/src/pages/pinned/index.vue`
- 检索与复用策略：
  - 检索关键词：置顶 / pinned / PostCard / postStore / pinned page
  - 找到的旧实现：首页置顶标识、置顶页时间轴布局
  - 最终选择：复用现有页面与组件，增量调整排序与时间显示
- 风险点：
  - 跨年份边界的年份卡片展示需确认
  - 小屏幕三列布局可能仍需微调
- 验证方式：
